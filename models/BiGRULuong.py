...
class Encoder(nn.Module):
  def __init__(self, input_size, embedding_size, hidden_size, num_layers, p, pretrained_word_embedding=False, embedding_matrix=None, freeze=False):
    ...
    self.fc_encoder_states = nn.Linear(hidden_size*2, hidden_size)
  ...
  def forward(self, x, lens):
    ...
    # hidden : [2, N, hidden_size] (bidirectional)

    hidden = torch.cat((hidden[0:1], hidden[1:2]), dim=2)
    # hidden : [2, N, hidden_size] ==> [1, N, hidden_size*2]

    hidden = self.fc_hidden(hidden)

    encoder_states, _ = pad_packed_sequence(encoder_states)
    encoder_states = self.fc_encoder_states(encoder_states)
...
class Decoder(nn.Module):
  def __init__(self, input_size, embedding_size, hidden_size, output_size, 
    ...
    self.W1 = nn.Linear(self.hidden_size*2, self.hidden_size)
  ...
  def forward(self, x, encoder_states, hidden):
    ...
    encoder_states_transposed = torch.transpose(encoder_states, 0, 1)
    encoder_states_transposed = encoder_states_transposed.permute(1, 2, 0)
    
    attention_score = torch.matmul(hidden_with_time_axis, encoder_states_transposed).permute(0, 2, 1)
...
class Seq2Seq(nn.Module):
  ...
  def forward(self, source, target, input_len, teacher_force_ratio=.5):
    ...
    encoder_states, hidden = self.encoder(source, input_len)
    # ambil start token
    x = target[0]
